...
def setup_survival_import():
    try:
        ...
        error_msg = str(e)
        if 'cblas' in error_msg.lower() or 'blas' in error_msg.lower() or 'undefined symbol' in error_msg.lower():
            raise ImportError(
                f"Failed to import survival module: {error_msg}\n"
                "This usually means BLAS libraries are missing.\n"
                "Install BLAS: sudo pacman -S openblas (Arch) or sudo apt-get install libopenblas-dev (Ubuntu/Debian)"
            ) from e
        pass
